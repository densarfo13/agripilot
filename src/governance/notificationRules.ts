 ...
 *     'src/governance/notificationRules';
 *
...
export interface NotificationCandidate {
  ...
  /** driven by weather / scan / care signal */
  readonly contextual?: boolean;
  readonly weatherAware?: boolean;
  readonly growthAware?: boolean;
}
...
export const FREQUENCY_LIMITS: Readonly<Record<NotificationChannel, ChannelLimit>> = Object.freeze({
...
// slip through.
export const FORBIDDEN_NOTIFICATION_PATTERNS: ReadonlyArray<ForbiddenPattern> = Object.freeze([
...
] as ReadonlyArray<ForbiddenPattern>);

const VALID_CHANNELS = new Set<string>(['push', 'sms', 'whatsapp', 'email']);

/**
 * Validate a single notification candidate against the rules.
 * Returns { ok, reasons } — never throws.
 */
export function validateNotification(input: NotificationCandidate | null | undefined): NotificationDecision {
  const safe = (input && typeof input === 'object') ? input : ({} as NotificationCandidate);
