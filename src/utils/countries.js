/**
 * Full country dataset — ISO 3166-1 alpha-2 codes, display names, and dial codes.
 * Used by CountrySelect component across all forms.
 *
 * Fields per entry:
 *   iso2     — stable ISO 3166-1 alpha-2 code (stored in DB as countryCode)
 *   name     — display name in English
 *   dialCode — E.164 dial code prefix (e.g. "+254" for Kenya)
 *
...
/** Look up a country by ISO2 code. Returns undefined if not found. */
export function getCountry(iso2) {
  if (!iso2) return undefined;
  return COUNTRIES.find(c => c.iso2 === iso2.toUpperCase());
}

/** Get display name for an ISO2 code, with a safe fallback. */
export function getCountryName(iso2) {
  const c = getCountry(iso2);
  return c ? c.name : iso2 || '—';
}

/** Get dial code for an ISO2 code (e.g. "+254"). Returns '' if not found. */
