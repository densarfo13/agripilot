/**
 * Farm Risks Route — GET /api/v2/farm-risks/:farmId
 *
 * Returns pest/disease risks for a specific farm.
 * Ownership enforced: farm must belong to req.user.id.
 * Risks are computed on-the-fly from rules — no database writes.
 * Uses crop, stage, seasonal timing, and weather for accurate risk assessment.
 */
 ...
 * GET /:farmId
 *
 * Returns generated pest/disease risks for a farm.
 */
router.get('/:farmId', authenticate, async (req, res) => {
  try {
    ...
    if (farm.status === 'archived') {
      return res.json({ success: true, risks: [], farmId: farm.id, message: 'Archived farms have no active risks' });
    }
    ...
    // Fetch weather (non-blocking — risks still work without weather)
    let weatherCtx = null;
    try {
      ...
      console.error('Weather fetch failed for risk generation (non-blocking):', weatherErr.message);
    }
    ...
    console.error('GET /api/v2/farm-risks/:farmId failed:', error);
    return res.status(500).json({ success: false, error: 'Failed to generate farm risks' });
  }
});
