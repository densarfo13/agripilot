...
// ---------------------------------------------------------------------------
router.get('/regions/risk', async (_req: Request, res: Response) => {
  try {
    ...
    const data = Array.from(byRegion.values()).map((s: any) => ({
      ...s,
...
// ---------------------------------------------------------------------------
router.get('/outbreak-clusters', async (req: Request, res: Response) => {
  try {
...
// ---------------------------------------------------------------------------
router.get('/farms/high-risk', async (req: Request, res: Response) => {
  try {
...
// ---------------------------------------------------------------------------
router.get('/interventions/effectiveness', async (_req: Request, res: Response) => {
  try {
    ...
    // ── By treatment type ──
    const typeMap: Record<string, { count: number; resolved: number; daySum: number; resolvedCount: number }> = {};
    for (const o of outcomes) {
      const action = (o as any).treatmentAction;
      const type = action?.actionTaken || 'unknown';
      if (!typeMap[type]) typeMap[type] = { count: 0, resolved: 0, daySum: 0, resolvedCount: 0 };
      typeMap[type].count++;
    ...
    // ── By crop ──
    const cropMap: Record<string, { count: number; resolved: number; treatments: Record<string, number> }> = {};
    for (const o of outcomes) {
      const action = (o as any).treatmentAction;
      const crop = action?.profile?.crop || 'unknown';
      const type = action?.actionTaken || 'unknown';
      if (!cropMap[crop]) cropMap[crop] = { count: 0, resolved: 0, treatments: {} };
      cropMap[crop].count++;
    ...
    const repeats = recentReports.filter((r: any) => r._count > 1).length;
...
// ---------------------------------------------------------------------------
router.patch('/hotspots/:id/status', async (req: Request, res: Response) => {
  try {
    const user = (req as AuthRequest).user;
...
// ---------------------------------------------------------------------------
router.post(
  '/boundaries/:profileId/validate',
  validate(validateBoundarySchema),
...
// ---------------------------------------------------------------------------
router.get('/queues/false-positive', async (_req: Request, res: Response) => {
  try {
...
// ---------------------------------------------------------------------------
router.get('/queues/boundary-review', async (_req: Request, res: Response) => {
  try {
    ...
    const data = boundaries.map((b: any) => ({
      id: b.id,
...
// ---------------------------------------------------------------------------
router.get('/queues/alert-review', async (_req: Request, res: Response) => {
  try {
...
// ---------------------------------------------------------------------------
router.get('/queues/summary', async (_req: Request, res: Response) => {
  try {
