 ...
 */
export async function ingestDroneScan(data) {
    try {
        ...
        if (data.hotspotZoneId) {
            ...
            if (!hotspot) {
                console.warn(`[Drone] Hotspot zone ${data.hotspotZoneId} not found — skipping validation`);
                return { scan };
            }
            ...
            if (validationScore > 70) {
                ...
                console.log(`[Drone] Hotspot ${hotspot.id} CONFIRMED (score: ${validationScore})`);
            }
            else if (validationScore < 40) {
                // False alarm: drone evidence contradicts satellite signal
                newStatus = 'false_alarm';
                console.log(`[Drone] Hotspot ${hotspot.id} marked FALSE ALARM (score: ${validationScore})`);
            }
            else {
                // Inconclusive: mark as inspected for follow-up
                newStatus = 'inspected';
                console.log(`[Drone] Hotspot ${hotspot.id} marked INSPECTED (score: ${validationScore})`);
            }
 ...
 */
export async function getDroneScans(profileId, limit = 10) {
    try {
