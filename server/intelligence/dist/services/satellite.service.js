 ...
 */
export async function ingestSatelliteScan(data) {
    try {
        // 1. Reject unusable scans
        const cloudCover = data.cloudCover ?? 0;
        if (cloudCover > 80) {
            throw new Error(`[Satellite] Cloud cover ${cloudCover}% exceeds 80% threshold — scan rejected`);
        }
        ...
        const anomalyScore = clamp(stressScore > 50 ? anomalyBase + (stressScore - 50) * 0.5 : anomalyBase);
        ...
        if (previousStress) {
            temporalChangeScore = clamp(Math.abs(stressScore - previousStress.stressScore) * 2);
        }
        ...
        console.log(`[Satellite] Stress scores — stress: ${stressRecord.stressScore}, ` +
        ...
        if (anomalyScore > 60) {
            ...
            console.log(`[Satellite] Auto-generated hotspot ${hotspot.id} — severity: ${severity}`);
        }
