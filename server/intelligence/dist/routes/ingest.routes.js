...
import { validate, ingestSatelliteSchema, ingestDroneSchema, } from '../validation/schemas.js';
...
router.post('/score/farm', async (req, res) => {
    try {
        ...
        const imageScores = recentImages.flatMap((img) => (img.detections || []).map((d) => d.confidence || 0));
        ...
        if (recentReport?.verificationAnswers) {
            const answerMap = {};
            for (const a of recentReport.verificationAnswers) {
