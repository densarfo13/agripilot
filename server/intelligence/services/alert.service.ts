 ...
 */
export function downgradeAlertLevel(level: DbAlertLevel): DbAlertLevel | null {
  const map: Record<DbAlertLevel, DbAlertLevel | null> = {
    urgent: 'high_risk',
    high_risk: 'elevated',
    elevated: 'watch',
    watch: null,
...
export async function evaluateAndCreateAlert(
  ...
  try {
    ...
    if (existingDuplicate) {
      ...
      return {
        created: false,
        alert: existingDuplicate as Record<string, unknown>,
        suppressed: true,
    ...
    if (targetType === 'farm') {
      ...
      if (recentAction) {
        console.log(
          `${LOG_PREFIX} Suppressed: recent treatment action for farm ${targetId}`,
        );
        return {
          created: false,
          alert: null,
    ...
    console.log(
      `${LOG_PREFIX} Created alert id=${alert.id} level=${effectiveLevel} confidence=${confidenceScore}`,
    );
...
export async function getActiveAlerts(
  ...
  try {
    const alerts = await (prisma as any).v2AlertEvent.findMany({
      where: {
        targetType,
        targetId,
        sentStatus: { in: ['pending', 'sent'] },
    ...
    console.log(
      `${LOG_PREFIX} Retrieved ${alerts.length} active alerts for ${targetType}/${targetId}`,
    );
 ...
 */
export async function getAlertsByFarmer(
  userId: string,
  limit: number = 20,
): Promise<Record<string, unknown>[]> {
  try {
    // Fetch the farmer's farm profile IDs so we can include farm-level alerts.
    const farmProfiles = await (prisma as any).farmProfile.findMany({
      where: { userId },
      select: { id: true },
    });

    const farmProfileIds: string[] = farmProfiles.map(
      (fp: { id: string }) => fp.id,
    );

    const combinedAlerts = await (prisma as any).v2AlertEvent.findMany({
      where: {
        AND: [
          {
            OR: [
    ...
    console.log(
      `${LOG_PREFIX} Retrieved ${combinedAlerts.length} alerts for farmer ${userId}`,
    );
    return combinedAlerts as Record<string, unknown>[];
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
...
export async function suppressAlert(
  ...
  try {
    const alert = await (prisma as any).v2AlertEvent.update({
      where: { id: alertId },
      data: {
...
export async function expireStaleAlerts(): Promise<number> {
  try {
    const result = await (prisma as any).v2AlertEvent.updateMany({
      where: {
